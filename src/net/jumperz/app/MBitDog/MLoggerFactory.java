...
public class MLoggerFactory {
	...
	// --------------------------------------------------------------------------------
	public static List load(String configFileName) throws IOException {
		/*
		 * <logger> fileName=work/bitdog/access pattern=[^:]{1,}:[0-9] condition=match case_sensitive=yes type=sizen rotate=104857600 eat=yes suffix=0000 command=/bin/gzip buffering=true //option buffer_size=0 //option timestamp=no //option </logger>
		 */
		...
		try {
			...
			for (int lineNumber = 1;; ++lineNumber) {
				...
				if (line.equalsIgnoreCase("<logger>")) {
					if (fileName != null || patternStr != null || conditionStr != null || caseSensitiveFlagStr != null || type != null || rotateStr != null || eatFlagStr != null || suffix != null || command != null || rotate != 0) {
					...
					/*
					 * if( rotate < 1 ) { throwParseError( configFileName, lineNumber ); }
					 */
					...
					/*
					 * if( !suffix.matches( "^0{1,}$" ) ) { throwParseError( configFileName, lineNumber ); }
					 */
	...
	// --------------------------------------------------------------------------------------
	private static void throwParseError(String configFileName, int lineNumber) throws IOException {
