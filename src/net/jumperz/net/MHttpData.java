...
public abstract class MHttpData {
	...
	public synchronized final void setHeaderValue(String name, String value) {
		...
		/*
		 * removeHeaderValue( name ); headerList.add( name + ": " + value );
		 */
	}
	...
	// -------------------------------------------------------------------------------------------
	protected final void readHeaderFields(MLineReader reader) throws IOException {
		...
		/*
		 * old code =) headerStr = headerStr.replaceAll( "\r\n ", "\r " ); headerStr = headerStr.replaceAll( "\r\n\t", "\r\t" ); String[] headerArray = headerStr.split( "\r\n" );
		 * 
		 * for( int i = 0; i < headerArray.length; ++i ) { String workStr = headerArray[ i ].replaceAll( "\r\t" , "\r\n\t" ); workStr = workStr.replaceAll( "\r ", "\r\n " ); if( workStr.indexOf( ":" ) == -1 ) { System.err.println( workStr ); throw new IOException( "Invalid HTTP data" ); } headerList.add( workStr ); }
		 */
	}

	// -------------------------------------------------------------------------------------------
	protected final void recvBody() throws IOException {
	...
	// --------------------------------------------------------------------------------
	protected int getContentLength() throws MHttpIOException {
		int contentLength = -1;
	 ...
	 */
	private final String readTrailer(BufferedInputStream inputStream, OutputStream bodyBuffer) throws IOException {
	 ...
	 * Caution: inputStream must reset, skipped and marked
	 */
	private final int readChunkSize(BufferedInputStream inputStream, OutputStream bodyBuffer) throws IOException {
		MLineReader reader = getLineReader();
		reader.setInputStream(inputStream);
	...
	// -------------------------------------------------------------------------------------------
	private final void readCRLF(InputStream inputStream, OutputStream bodyBuffer) throws IOException {
	...
	// --------------------------------------------------------------------------------
	public final void setBodyBuffer(MBuffer newBuffer) throws IOException {
		clearFromInside();
	...
	// --------------------------------------------------------------------------------
	public final void setBody(byte[] data) throws IOException {
		clearFromInside();
	...
	// --------------------------------------------------------------------------------
	public final void setBody(String str, String charset) throws IOException {
