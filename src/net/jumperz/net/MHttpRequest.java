...
public final class MHttpRequest extends MHttpData implements Cloneable {
	...
	// --------------------------------------------------------------------------------
	public MHttpRequest(String s) throws IOException {
		BufferedInputStream i = new BufferedInputStream(new ByteArrayInputStream(s.getBytes(MCharset.CS_ISO_8859_1)));
		init(i);
	}

	// --------------------------------------------------------------------------------
	public MHttpRequest(byte[] buffer) throws IOException {
		BufferedInputStream i = new BufferedInputStream(new ByteArrayInputStream(buffer));
		init(i);
	}

	// --------------------------------------------------------------------------------
	public MHttpRequest(BufferedInputStream in_bufferedInputStream) throws IOException {
		init(in_bufferedInputStream);
	}

	// -------------------------------------------------------------------------------
	public void init(BufferedInputStream in_bufferedInputStream) throws IOException {
	...
	// --------------------------------------------------------------------------------
	public String getSessionId(int sessionType, String sessionIdName) throws IOException {
	...
	public void parseParameters() {
		...
		if (hasBody()) {
			if (headerExists("Content-Type")) {
				...
				} else if (contentType.toLowerCase().indexOf("/json") > -1 || contentType.toLowerCase().indexOf("-json") > -1) {
					bodyType = BODY_TYPE_JSON;
				} else if (contentType.toLowerCase().indexOf("/xml") > -1 || contentType.toLowerCase().indexOf("-xml") > -1) {
	...
	// --------------------------------------------------------------------------------
	private void parseParameters3(String boundary) throws IOException {
	...
	// --------------------------------------------------------------------------------
	private void splitRequestLine2(String line) throws IOException {
	...
	// --------------------------------------------------------------------------------
	private void splitRequestLine(String line) throws IOException {
		// block "GET / HTTP/1.0 "
	...
	// --------------------------------------------------------------------------------
	public static void testHasValidBodyState() throws Exception {
		...
		if (request.isInvalidPostState() != true) {
			throw new Exception();
		}

		request.setBody("i=5&bazz=%82%A0&title=&message=");

		if (request.isInvalidPostState() != false) {
			throw new Exception();
		}
	...
	public boolean isKeepAliveRequest(String fieldName) {
		...
		if (version.equals("HTTP/1.1")) {
			...
			if (headerExists(fieldName) && getHeaderValue(fieldName).equalsIgnoreCase("close")) {
				keepAlive = false;
			}
		} else {
			keepAlive = false;

			if (headerExists(fieldName) && getHeaderValue(fieldName).equalsIgnoreCase("Keep-Alive")) {
				keepAlive = true;
			}
		}
		return keepAlive;
	...
	// --------------------------------------------------------------------------------
	/*
	 * public boolean hasParameter( String name ) { return parameterNameList.contains( name ); }
	 */
	// --------------------------------------------------------------------------------
	...
	/*
	 * // -------------------------------------------------------------------------------- public List getParameterNameList() { return parameterNameList; }
	 */
	/*
	 * // -------------------------------------------------------------------------------- public void setParameter( MAbstractParameter param, int index ) { String name = param.getName(); if( parameterNameSet1.contains( name ) ) { parameterMap1.put( name, param ); } else if( parameterNameSet2.contains( name ) ) { List l = ( List )parameterMap2.get( name ); for( int i = 0; i < l.size(); ++i ) { if( i
	 * == index ) { //replace old param Object oldParam = l.get( index ); l.add( index, param ); l.remove( oldParam ); } } } else { addParameter( param ); } } // -------------------------------------------------------------------------------- public void setParameter( MAbstractParameter param ) { setParameter( param, 0 ); }
	 */
	/*
	 * // -------------------------------------------------------------------------------- public List getParameterList( String name ) { if( parameterNameSet1.contains( name ) ) { List l = new ArrayList(); l.add( parameterMap1.get( name ) ); return l; } else if( parameterNameSet2.contains( name ) ) { return ( List )parameterMap2.get( name ); } else { return new ArrayList(); } } //
	 * -------------------------------------------------------------------------------- public MAbstractParameter getParameter( String name ) { if( parameterNameSet1.contains( name ) ) { return ( MAbstractParameter )parameterMap1.get( name ); } else if( parameterNameSet2.contains( name ) ) { return ( MAbstractParameter )( ( List )parameterMap2.get( name ) ).get( 0 ); } else { return new MParameter(
	 * name, "", MAbstractParameter.URI ); } }
	 */
	// --------------------------------------------------------------------------------
	...
	// --------------------------------------------------------------------------------
	public void setParameterList(List l, int type) throws IOException {
		if (type == MAbstractParameter.URI) {
			uriParameterList = l;
		} else if (type == MAbstractParameter.BODY) {
			bodyParameterList = l;
	...
	// --------------------------------------------------------------------------------
	public void applyParameterList(int type) throws IOException {
		if (type == MAbstractParameter.URI) {
