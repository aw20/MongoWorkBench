...
public final class MHttpResponse extends MHttpData implements Cloneable {
	...
	// --------------------------------------------------------------------------------
	public MHttpResponse(byte[] buffer) throws IOException {
	...
	// -------------------------------------------------------------------------------------------
	public MHttpResponse(BufferedInputStream in) throws IOException {
		hasBodyFlag = true;
		init(in);
	}

	// -------------------------------------------------------------------------------------------
	public MHttpResponse(BufferedInputStream in, boolean isResponseOfHeadMethod) throws IOException {
		init(in, isResponseOfHeadMethod);
	}

	// --------------------------------------------------------------------------------
	public MHttpResponse(String s) throws IOException {
	...
	// --------------------------------------------------------------------------------
	public void init(BufferedInputStream in, boolean isResponseOfHeadMethod) throws IOException {
		hasBodyFlag = !isResponseOfHeadMethod;
		init(in);
	}

	// --------------------------------------------------------------------------------
	public void init(BufferedInputStream in) throws IOException {
	...
	// --------------------------------------------------------------------------------
	public void setStatusLine(String line) throws IOException {
		splitStatusLine(line);
	}

	// --------------------------------------------------------------------------------
	private void splitStatusLine(String line) throws IOException {
		...
		/*
		 * String[] statusLineArray = line.split( " " ); if( statusLineArray.length < 3 ) { throw new MHttpIOException( "Invalid status line:" + line ); }
		 * 
		 * version = statusLineArray[ 0 ];
		 * 
		 * statusCode = Integer.parseInt( statusLineArray[ 1 ] );
		 * 
		 * if( statusCode < 200 || statusCode == 304 || statusCode == 204 ) { hasBodyFlag = false; }
		 * 
		 * StringBuffer strBuf = new StringBuffer(); for( int i = 2; i < statusLineArray.length; ++i ) { if( i != 2 ) { strBuf.append( " " ); } strBuf.append( statusLineArray[ i ] ); } reasonPhrase = strBuf.toString();
		 */
	}
	...
	public boolean isKeepAliveResponse(String connHeaderName) {
		if (version.equals("HTTP/1.0")) {
			if (headerExists(connHeaderName) && getHeaderValue(connHeaderName).equalsIgnoreCase("Keep-Alive") && headerExists("Content-Length")) {
				return true;
			} else {
				return false;
			}
			...
			if (headerExists(connHeaderName)) {
				...
				return true;
			} else if (headerExists("Transfer-Encoding") && getHeaderValue("Transfer-Encoding").equalsIgnoreCase("chunked")) {
				return true;
			} else {
				// no header fields
				if (hasBody()) {
	...
	public void gzip() throws IOException {
		...
		MStreamUtil.connectStream(in, new GZIPOutputStream(buf), true);
		setBodyBuffer(buf);
