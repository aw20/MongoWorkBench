...
public class MBlowfishCipher implements MBlowfishECBCryptInfo {
	...
	// --------------------------------------------------------------------------------
	public synchronized String decryptToString(byte[] data) throws IOException {
		byte[] dec = decrypt(data);
		return MStringUtil.byteArrayToString(dec);
	}

	// --------------------------------------------------------------------------------
	public synchronized byte[] encryptToBytes(String s) throws IOException {
		byte[] data = MStringUtil.getBytes(s);
		return encrypt(data);
	}

	// --------------------------------------------------------------------------------
	public synchronized String encrypt(String s) throws IOException {
		return MStringUtil.byteToHexString(encryptToBytes(s));
	}

	// --------------------------------------------------------------------------------
	public synchronized String decrypt(String s) throws IOException {
