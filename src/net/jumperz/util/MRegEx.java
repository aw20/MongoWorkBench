...
public class MRegEx {
	...
	public static void main(String[] args) throws Exception {
		...
		if (containsIgnoreCase("hoge fuga", "Hoge" + WORD_BETWEEN + "fuga") == false) {
			throw new Exception();
		}
		if (containsIgnoreCase("hoge  fuga", "Hoge" + WORD_BETWEEN + "fuga") == false) {
			throw new Exception();
		}
		if (containsIgnoreCase("hoge/*dummy*/fuga", "Hoge" + WORD_BETWEEN + "fuga") == false) {
			throw new Exception();
		}
		if (containsIgnoreCase("hoge gyoe  fuga", "Hoge" + WORD_BETWEEN + "fugA") == false) {
			throw new Exception();
		}
		if (containsIgnoreCase("hogeafuga", "Hoge" + WORD_BETWEEN + "fuga") == true) {
			throw new Exception();
		}
		if (containsIgnoreCase("hoge", WORD_HEAD + "hoge") == false) {
			throw new Exception();
		}
		if (containsIgnoreCase("Ahoge", WORD_HEAD + "hoge") == true) {
			throw new Exception();
		}
		if (containsIgnoreCase("x hoge", WORD_HEAD + "hoge") == false) {
			throw new Exception();
		}
	...
	private static final void testReplaceAll() throws Exception {
		...
		if (!replaceAllIgnoreCase("A**A", "\\*", "x").equals("AxxA")) {
	...
	public static String replaceAllIgnoreCase(String target, String regex, String to) {
		...
		/*
		 * int len = regex.length(); if( len == 0 ) { return target; } StringBuffer buf = new StringBuffer( target.length() ); while( true ) { String matchStr = getMatch( regex, target ); len = matchStr.length(); if( len == 0 ) { break; } int pos = target.indexOf( matchStr ); buf.append( target.substring( 0, pos ) ); buf.append( to ); target = target.substring( pos + len ); } buf.append( target );
		 * System.out.println( buf.toString() ); return buf.toString();
		 */
	}
